...
/* al Plugins By: LA BANDA 2020*/
#define PLUGINNAME        "[ZP] Clases de Zombie "
#define VERSION           "2.0"
...
// Modifica a tu gusto ZP, ZKG, ZPA
#include <zombieplague>

// Plugin puede ser usado por zp, zpadvance, y zp normal.
// 
#if defined _zombie_killer_galaxy_included
#define TAG "^x04[ZKG]^x01"
#define is_zombie(%1)	(zp_get_user_zombie(%1) && !zp_get_user_assassin(%1) && zp_get_user_nemesis(%1) && !zp_get_user_alien(%1))
#define is_human(%1)	(!zp_get_user_zombie(%1) && zp_get_user_survivor(%1) && !zp_get_user_depre(%1) && !zp_get_user_sniper(%1) && !zp_get_user_l4d(%1, BILL) && !zp_get_user_l4d(%1, FRANCIS) && !zp_get_user_l4d(%1, LOUIS)  && !zp_get_user_l4d(%1, ZOEY))
#endif

#if defined _zombie_plague_advance_included
#define TAG "^x04[ZPA]^x01"
#define is_zombie(%1)	(zp_get_user_zombie(%1) && !zp_get_user_nemesis(%1) && !zp_get_user_assasin(%1))
#define is_human(%1)	(!zp_get_user_zombie(%1) && !zp_get_user_survivor(%1) && !zp_get_user_sniper(%1))
#endif

#if defined _zombieplague_included
#define TAG "^x04[ZP]^x01"
#define is_zombie(%1)	(zp_get_user_zombie(%1) && !zp_get_user_nemesis(%1))
#define is_human(%1)	(!zp_get_user_zombie(%1) && !zp_get_user_survivor(%1))
#endif
...
public plugin_precache() 
{
	...
	for(i = 0; i < sizeof ZCLASSES; i++)
		g_zclass[i] = zp_register_zombie_class(ZCLASSES[i][NAME], ZCLASSES[i][INFO], ZCLASSES[i][PLAYMDL], ZCLASSES[i][KNIFEMDL], ZCLASSES[i][HEALTH], ZCLASSES[i][SPEED], ZCLASSES[i][GRAVITY], ZCLASSES[i][KB])
...
public zp_user_infected_post(id, infector)
{
	...
	else if(Class == g_zclass[FLESHPOUND])
	{
	...
	else if(Class == g_zclass[BOOMER2])
	{
...
{ 	
	...
	if (zp_get_user_zombie_class(victim) != g_zclass[BOOMER])
		return HAM_IGNORED;
...
public fw_PlayerSpawn_Post(id)
{
	...
	if(zp_get_user_zombie_class(id) == g_zclass[DEIMOS] && is_zombie(id))
		g_cd[id] = true
...
public fw_PlayerKilled(victim, attacker, shouldgib)
{
	...
	if(zp_get_user_zombie_class(victim) == g_zclass[STALKER])
...
public effect_fire(entity)
{
	...
	// Colored Aura
	message_begin(MSG_PAS, SVC_TEMPENTITY, originF, 0)
	write_byte(TE_DLIGHT) 			// TE id
	write_coord( originF[0])	// x
	write_coord( originF[0])	// y
	write_coord( originF[0])	// z
...
{	
	...
	if(!(is_zombie(iVictim) && is_human(iAttacker)))
		return HAM_IGNORED;
	
	if(zp_get_user_zombie_class(iVictim) == g_zclass[BLINDADO])
	{
		new wpn_attacker = get_user_weapon(iAttacker)
		if (wpn_attacker == CSW_SCOUT || wpn_attacker == CSW_XM1014
		|| wpn_attacker == CSW_MAC10|| wpn_attacker == CSW_AUG
		|| wpn_attacker == CSW_UMP45 || wpn_attacker == CSW_SG550
		|| wpn_attacker == CSW_GALIL || wpn_attacker == CSW_FAMAS
		|| wpn_attacker == CSW_MP5NAVY || wpn_attacker == CSW_AWP
		|| wpn_attacker == CSW_M249 || wpn_attacker == CSW_M3
		|| wpn_attacker == CSW_M4A1 || wpn_attacker == CSW_TMP
		|| wpn_attacker == CSW_G3SG1 || wpn_attacker == CSW_SG552
		|| wpn_attacker == CSW_AK47 || wpn_attacker == CSW_P90)
		{			
...
public client_PostThink(id)
{
	...
	if(zp_get_user_zombie_class(id) != g_zclass[THERMAL])
		return PLUGIN_CONTINUE
...
public fw_TakeDamage(victim, inflictor, attacker, Float:damage, damage_type)
{
	...
	if(zp_get_user_zombie_class(attacker) == g_zclass[CLOT] && is_zombie(attacker))
	{
		if(is_human(victim))
		{
	...
	else if(zp_get_user_zombie_class(victim) == g_zclass[SCRAKE] && is_zombie(victim) && !zp_get_user_zombie(attacker))
	{
...
drop_weapon(id)
{
	...
	if(is_user_alive( target ) && is_human(target))
	{	
		message_begin(MSG_BROADCAST ,SVC_TEMPENTITY)
		write_byte(TE_EXPLOSION)
...
public fw_PlayerPreThink(id)
{
	...
	if(g_zclass[CHARGER] == Class)
	{
		set_pev(id, pev_maxspeed, g_speeded[id] ? get_pcvar_float(cvar_speed) : float(ZCLASSES[CHARGER][SPEED]))
	}
	else if(g_zclass[FLESHPOUND] == Class)
	{
		set_pev(id, pev_maxspeed, g_speeded[id] ? 600.0 : float(ZCLASSES[FLESHPOUND][SPEED]))
		if (!(iOldButton & IN_RELOAD) && (iButton & IN_RELOAD))
			clcmd_furia(id)	
	}
	else if(g_zclass[SCRAKE] == Class)
	{
...
public fw_CmdStart(id, handle, random_seed)
{
	...
	else if(Class == g_zclass[HOUDEYE])
	{
...
public zp_user_unfrozen(id)
{
	...
	if (zp_get_user_zombie_class(id) != g_zclass[STALKER])
		return PLUGIN_CONTINUE
...
public use_ability_one(id)
{
	...
	if(zp_get_user_zombie_class(id) != g_zclass[CHARGER])
		return PLUGIN_HANDLED
...
public clcmd_furia(id)
{
	...
	if(zp_get_user_zombie_class(id) != g_zclass[FLESHPOUND])
		return PLUGIN_HANDLED
...
public clcmd_vomit( id )
{
	...
	if(zp_get_user_zombie_class(id) != g_zclass[BOOMER2])
		return PLUGIN_HANDLED
